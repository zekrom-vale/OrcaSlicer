...
"    'f' replaces only the first match within each chunk (layer or color block),\n"
"    which means the same substitution can fire once per chunk — effectively\n"
"    duplicating the action across chunks.\n\n"
"BLOCK FLAGS\n"
"═══════════════════════════════════════════════════════════\n"
" DEFAULT = layer block — substitution is applied per layer chunk. The G-code is split\n"
...
"       (after ; EXECUTABLE_BLOCK_END) are also treated as layer chunks.\n\n"
"  C  = color/toolhead block — substitution is applied per color chunk within\n"
"       each layer. Color chunks are delimited by bare T commands (T0, T1, T2, etc.).\n"
...
"    Rules without C flag are layer-level rules (applied per layer chunk). This is the default.\n\n"
"  M  = metadata-only — rule applies only to preamble/suffix chunks (outside layers).\n"
"       Non-M rules apply only to layer/color chunks. This separation prevents\n"
"       rules from matching in the wrong chunk type.\n\n"
"  P  = preprint-only — rule applies only to the PrePrint section, which is the\n"
"       machine initialization block between ; EXECUTABLE_BLOCK_START and the first\n"
"       ;LAYER_CHANGE. This section contains G-code like M104, M109, M83, T0, etc.\n"
"       that runs before any model geometry. The {layer_num} macro resolves to -1\n"
"       in PrePrint scope to distinguish it from layer 0.\n\n"
"    ⚠ M, P, and C are mutually exclusive section-mode flags. Combining any two\n"
"      produces a runtime error. M targets preamble/suffix, P targets PrePrint,\n"
"      and C targets color chunks within layers.\n\n"
"  R  = run/don't run condition — rule is only applied when the boolean expression\n"
"       evaluates to true. The condition is evaluated per chunk using the current\n"
"       scope variables (layer_num, layer_z, current_extruder, etc.). This prevents\n"
"       expensive regex evaluations on chunks where the rule doesn't apply.\n\n"
"    Format: R{boolean_expression}\n\n"
"    Examples:\n"
"      R{layer_num > 5}                    Only layers 6 and above\n"
"      R{first_layer}                      First layer only\n"
"      R{!first_layer && !last_layer}      Middle layers only\n"
"      R{current_extruder == 0}            Extruder 0 only\n"
"      R{layer_num > 5 && current_extruder == 0}  Combined condition\n\n"
"    The condition uses the same boolean expression syntax as PlaceholderParser:\n"
"      - Comparison: ==, !=, <, >, <=, >=\n"
"      - Logical: &&, ||, !\n"
"      - Ternary: condition ? true_value : false_value\n"
"      - Functions: min(), max(), int(), round(), ceil(), floor()\n\n"
"    ⚠ R is orthogonal to M/P/C — it can be combined with any section-mode flag.\n"
"      The condition is evaluated first, then section-mode filtering applies.\n\n"
"CHUNKING DETAILS\n"
"═══════════════════════════════════════════════════════════\n"
"  G-code is split into chunks at these boundaries:\n\n"
"  1. ; EXECUTABLE_BLOCK_START — starts the PrePrint section. The PrePrint\n"
"     section contains machine initialization G-code (M104, M109, M83, T0, etc.)\n"
"     that runs before any model geometry. The first ;LAYER_CHANGE after\n"
"     EXECUTABLE_BLOCK_START ends the PrePrint section and starts layer 0.\n\n"
"  2. ;LAYER_CHANGE / ; CHANGE_LAYER — starts a new layer chunk (after layer 0).\n"
"     The match line belongs to the NEW layer (not the prior one).\n\n"
"  3. T0, T1, T2, ... — starts a new color chunk within the current layer.\n"
"     The match line belongs to the NEW color chunk (not the prior one).\n\n"
"  4. ; EXECUTABLE_BLOCK_END — ends the last layer and starts the suffix.\n"
"     The match line belongs to the suffix (not the last layer).\n\n"
"  Chunk hierarchy: Preamble → PrePrint → Layer 0 → Layer 1 → ... → Last Layer → Suffix\n"
"  Each layer contains one or more color chunks (delimited by T commands).\n\n"
"  The preamble includes everything before ; EXECUTABLE_BLOCK_START.\n"
"  The PrePrint section includes everything between ; EXECUTABLE_BLOCK_START\n"
"  and the first ;LAYER_CHANGE.\n"
"  The suffix includes ; EXECUTABLE_BLOCK_END and everything after it.\n\n"
"  Rules are separated by chunk type:\n"
"  - Default and C rules: run ONLY on layer/color chunks (Ignore preamble, PrePrint, and suffix)\n"
"  - M-flagged rules: run ONLY on preamble/suffix (Ignore PrePrint and layer/color chunks)\n"
"  - P-flagged rules: run ONLY on PrePrint (Ignore preamble, suffix, and layer/color chunks)\n"
"  This strict separation prevents matching errors between chunk types.\n\n"
"═══════════════════════════════════════════════════════════\n"
"ESCAPE SEQUENCES\n"
"═══════════════════════════════════════════════════════════\n"
...
"PROCESSING\n"
"═══════════════════════════════════════════════════════════\n"
"  All rules are processed in a chunked full-file pass after the G-code file\n"
"  is written. The G-code is split at layer boundaries (; CHANGE_LAYER / ;LAYER_CHANGE)\n"
"  and color boundaries (; CP TOOLCHANGE START). Each chunk is processed\n"
"  independently, allowing multiline matches and newline insertion.\n\n"
"═══════════════════════════════════════════════════════════\n"
"CAPTURE GROUPS\n"
"═══════════════════════════════════════════════════════════\n"
...
"  l/foo\\nbar/baz/                  Literal match foo<newline>bar, replace with baz\n"
...
"MACRO VARIABLES\n"
"═══════════════════════════════════════════════════════════\n"
"Replacement strings can reference macro variables using {var} syntax.\n"
...
"PREPRINT-SCOPED VARIABLES (only in PrePrint chunks with P flag):\n"
"  {layer_num}            Always -1 in PrePrint (no layer context)\n"
"  {first_layer}          Always false in PrePrint\n"
"  {last_layer}           Always false in PrePrint\n\n"
"COLOR-SCOPED VARIABLES (only in color blocks):\n"
"  {color_chunk_num}      Current 1-based color chunk index\n"
"  {current_extruder}     Current tool/extruder ID\n\n"
...
"BRACE PROTECTION\n"
"═══════════════════════════════════════════════════════════\n"
"Literal braces in replacement strings are protected automatically:\n"
"  ${1}                    Regex back-reference — NOT treated as macro\n"
"  \\g{name}               Named group reference — NOT treated as macro\n"
"  \\{literal\\}            Escaped braces — passed through literally\n\n"
"═══════════════════════════════════════════════════════════\n"
"MACRO ERROR HANDLING\n"
"═══════════════════════════════════════════════════════════\n"
"If a macro variable is undefined (e.g., {layer_num} in preamble without\n"
"M flag), a warning is logged and the unresolved replacement string is\n"
"used as-is. The G-code export does NOT abort.\n\n"
"═══════════════════════════════════════════════════════════\n"
"MACRO EXAMPLES\n"
"═══════════════════════════════════════════════════════════\n"
"  s/;LAYER_START/;LAYER {layer_num}/M/          Insert layer number\n"
...
"  s/;COLOR/;COLOR {color_chunk_num}/C/          Insert color chunk number\n"
"  s/M83/M83 ; layer_num={layer_num}/P/          Insert layer_num in PrePrint (resolves to -1)\n"
